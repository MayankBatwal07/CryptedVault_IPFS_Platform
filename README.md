...
```bash
git clone https://github.com/MayankBatwal07/CryptedVault_IPFS_Platform.git
```

### Step 2: Setup Server Environment Variables

Navigate to the `server` directory in vaultBackend:

```bash
cd CryptedVault_IPFS_Platform/vaultBackend/server
```
...
Navigate to the `client` directory in VaultFrontend:

```bash
cd VaultFrontend/client
```
...
Now, you should be able to access the CryptedVault_IPFS_Platform application locally.
...
For any queries contact me at [LinkedIn](https://www.linkedin.com/in/mayankbatwal/)
